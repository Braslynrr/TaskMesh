    ...
    it("move list to 2nd position", async () => {
        ...
        const lists = await Promise.all(
            Array.from({ length: 4 }, (_, i) =>
                createListForTaskboard(taskboardId, `t${i + 1}`)))

        const orderedList = lists.sort((a, b) => a.position - b.position)

        const res = await request(app)
            .post("/api/list/move")
            .set("Cookie", `auth_token=${token}`)
            .send({
                _id: orderedList[3]._id,
        ...
        expect(res.body[1]).toMatchObject({
            position: 2,
            title: orderedList[3].title
        })

        expect(res.body[2]).toMatchObject({
            position: 3,
            title: orderedList[1].title
        })
